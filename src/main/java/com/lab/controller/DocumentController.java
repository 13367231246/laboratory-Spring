...
public class DocumentController {
    ...
    /**
     * 普通用户分页获取所有已发布的文档（不按类型筛选）
     */
    @GetMapping("/user/published")
    public Result<PageBean<Document>> listPublished(@RequestParam Integer pageNo,
                                                    @RequestParam Integer pageSize) {
        PageBean<Document> pageBean = documentService.listPublished(pageNo, pageSize);
        return Result.success(pageBean);
    }
