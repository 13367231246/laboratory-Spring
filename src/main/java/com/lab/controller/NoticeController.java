...
public class NoticeController {
    ...
    /**
     * 普通用户分页获取所有已发布且未过期的公告（不按类型筛选）
     */
    @GetMapping("/user/published")
    public Result<PageBean<Notice>> listPublished(@RequestParam Integer pageNo,
                                                  @RequestParam Integer pageSize) {
        PageBean<Notice> pageBean = noticeService.listPublished(pageNo, pageSize);
        return Result.success(pageBean);
    }
