...
public interface DocumentMapper {
    ...
    // 分页查询所有已发布的文档（普通用户用，不按类型筛选）
    @Select("select * from document where status = 1 order by update_time desc limit #{offset}, #{pageSize}")
    List<Document> findPublishedPage(@Param("offset") Integer offset, @Param("pageSize") Integer pageSize);

    // 统计已发布文档总数
    @Select("select count(*) from document where status = 1")
    Long countPublished();
