...
public interface LabScheduleMapper {
    ...
    /** 根据实验室ID查询启用中的时间规则（laboratory_ids 包含该 id 或为 all，status=1） */
    @Select("select * from lab_schedule where status = 1 " +
            "and (laboratory_ids = 'all' or find_in_set(#{laboratoryId}, replace(laboratory_ids, ' ', '')) > 0) " +
            "order by update_time desc, id desc")
    List<LabSchedule> findByLaboratoryIdAndStatus(@Param("laboratoryId") Integer laboratoryId);
